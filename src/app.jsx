...
import { loadAppData, saveAppData, createBackup, parseBackupPayload, normalizeProgress } from './lib/storage.js';
import {
  filterTasks, formatDate, getAnalytics, getDashboardStats, getDueInfo, getLevel, getNextLevelXp,
  getSessionXp, getTaskXp, isOverdue, makeTask, selectDailyMission, sortTasks, todayString,
  updateStreak, validateTaskInput
} from './lib/domain.js';
  ...
  return (
    <MotionConfig reducedMotion={data.preferences.motion === 'compact' ? 'always' : 'user'}>
      <AppShell page={page} profile={data.profile} progress={data.progress} notice={notice}>
        ...
        {page === 'settings' && <SettingsPage data={data} commit={commit} updatePreferences={updatePreferences} />}
      </AppShell>
    </MotionConfig>
  );
}
        ...
        <nav className={`navbar container ${menuOpen ? 'menu-open' : ''}`} aria-label="Navigasi utama">
          ...
          <div className="nav-links">
            {NAV_ITEMS.map(({ href, page: itemPage, label, Icon }) => (
              <a key={itemPage} className={`nav-link ${page === itemPage ? 'active' : ''}`} href={href} aria-current={page === itemPage ? 'page' : undefined} onClick={() => setMenuOpen(false)}>
    ...
    <motion.section ref={heroRef} className="home-hero" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.34 }}>
      <div className="home-hero-copy">
        ...
        <PageActions>
          ...
          <a className="btn btn-secondary" data-hero-item href="tasks.html"><ListChecks size={16} />Lihat task board</a>
        </PageActions>
...
function SettingsPage({ data, commit, updatePreferences }) {
  const [profile, setProfile] = useState(data.profile);
  const [status, setStatus] = useState({ text: '', error: false });
  const [confirm, setConfirm] = useState(null);
  const fileRef = useRef(null);
  useEffect(() => setProfile(data.profile), [data.profile]);
  const saveProfile = (event) => { event.preventDefault(); commit((current) => ({ ...current, profile: { name: profile.name.trim() || 'Pengguna', tagline: profile.tagline.trim() || 'Ruang produktif harian' } })); setStatus({ text: 'Profil tersimpan.', error: false }); };
