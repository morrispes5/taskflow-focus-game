...
export function TasksPage({ data, commit, toggleTask }) {
  ...
  const unarchived = data.tasks.filter((task) => !task.archived);
  const statusCounts = {
    all: unarchived.length,
    active: unarchived.filter((task) => !task.completed).length,
    completed: unarchived.filter((task) => task.completed).length
  };
  ...
  return <>
    <section className="task-capture card">
      <div className="capture-title">
        <div><p className="section-kicker">Tangkap cepat</p><h2>Apa yang ingin kamu selesaikan?</h2></div>
        <span className="shortcut-hint">Tekan N untuk mulai, Enter untuk tambah</span>
      </div>
      <form className="quick-add-form" onSubmit={addQuick}>
        <label className="sr-only" htmlFor="quick-task">Judul tugas baru</label>
        <input ref={quickInputRef} id="quick-task" className="input input-large" value={quickText} onChange={(event) => { setQuickText(event.target.value); setQuickError(''); }} placeholder="Contoh: Selesaikan outline presentasi" maxLength={120} aria-invalid={Boolean(quickError)} />
        <button className="btn btn-primary" type="submit"><Zap size={16} />Tambah cepat</button>
        <button className="btn btn-secondary" type="button" onClick={() => setDialogTask({})}><MoreHorizontal size={17} />Tambah detail</button>
      </form>
      {quickError && <p className="form-error" role="alert">{quickError}</p>}
    </section>
    <section className="toolbar-section">
      <div className="filter-tabs" role="group" aria-label="Filter status tugas">
        {[['all', 'Semua'], ['active', 'Aktif'], ['completed', 'Selesai']].map(([key, label]) => (
          <button key={key} className={`chip ${status === key ? 'active' : ''}`} type="button" onClick={() => setStatus(key)}>
            {label}<span>{statusCounts[key]}</span>
          </button>
        ))}
      </div>
      <span className="counter-badge"><strong>{statusCounts.active}</strong> aktif</span>
    </section>
    ...
    <section className="task-results card">
      <div className="list-summary" role="status" aria-live="polite">Menampilkan {visible.length} dari {data.tasks.length} tugas</div>
      {visible.length ? (
        ...
        <EmptyState
          title={data.tasks.length ? 'Tidak ada hasil' : 'Belum ada tugas'}
          message={data.tasks.length ? 'Coba ubah kata kunci atau filter agar daftar lain terlihat.' : 'Tulis misi pertamamu di bagian atas untuk mulai membangun momentum.'}
          action="Tambah tugas"
          onAction={() => setDialogTask({})}
        />
      )}
    </section>
