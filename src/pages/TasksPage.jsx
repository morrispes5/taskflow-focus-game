...
export function TasksPage({ data, commit, toggleTask }) {
  ...
  const saveTask = (input, id) => commit((current) => id ? ({ ...current, tasks: current.tasks.map((task) => task.id === id ? { ...task, ...input, text: input.text.trim(), dueDate: input.dueDate || null, dueTime: input.dueTime || null, category: input.category?.trim() || null, estimateMinutes: Number(input.estimateMinutes) || 25, courseId: input.courseId || null, meetingNumber: input.meetingNumber ? Number(input.meetingNumber) : null, updatedAt: Date.now() } : task) }) : ({ ...current, tasks: [makeTask(input), ...current.tasks] }), id ? 'Tugas diperbarui.' : 'Tugas ditambahkan.', id ? null : 'taskAdded');
