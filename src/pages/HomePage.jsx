...
import { ArrowRight, CalendarClock, Check, CirclePlay, Clock3, Flame, Gauge, ListChecks, Snowflake, Sparkles, Trophy, Zap } from 'lucide-react';
import { playHeroSequence } from '../motion/anime.js';
import { getCountdownLabel, getCourseProgress, getDashboardStats, getDueInfo, getNextLevelXp, getProfileRecommendations, getStreakFreezesRemaining, getTaskFocusMinutes, getTaskXp, getTodayAgenda, getUpcomingDeadlines, makeTask, selectDailyMission, selectReviewTask, sortTasks, todayString } from '../lib/domain.js';
...
export function HomePage({ data, commit, toggleTask }) {
  ...
  const saveTask = (input, id) => commit((current) => id ? ({ ...current, tasks: current.tasks.map((task) => task.id === id ? { ...task, ...input, category: input.category?.trim() || null, dueDate: input.dueDate || null, dueTime: input.dueTime || null, estimateMinutes: Number(input.estimateMinutes) || 25, courseId: input.courseId || null, meetingNumber: input.meetingNumber ? Number(input.meetingNumber) : null, updatedAt: Date.now() } : task) }) : ({ ...current, tasks: [makeTask(input), ...current.tasks] }), id ? 'Tugas diperbarui.' : 'Tugas ditambahkan.', id ? null : 'taskAdded');
  useEffect(() => {
  ...
  return <>
    ...
    <section className="home-grid">
       <motion.article className={`mission-card ${mission ? '' : 'mission-card-empty'} ${isReviewTarget ? 'mission-card-review' : ''}`} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.36 }}>
        <div className="mission-copy"><div className="card-topline"><span className="eyebrow">{isReviewTarget ? 'Review tugas' : 'Focus Run'}</span><span className="quiet-status"><span className="status-dot" />{isReviewTarget ? 'Tugas sudah selesai' : 'Siap dimulai'}</span></div>{mission ? <><h2>{mission.text}</h2><div className="mission-meta"><span className={`priority-badge priority-${mission.priority}`}>{mission.priority === 'high' ? 'Prioritas tinggi' : mission.priority === 'medium' ? 'Prioritas sedang' : 'Prioritas rendah'}</span><span>{getDueInfo(mission).label}</span>{mission.category && <span>{mission.category}</span>}</div><a className="btn btn-dark" href={`focus.html?intent=start&taskId=${mission.id}`}><CirclePlay size={18} fill="currentColor" />Mulai Focus Run<ArrowRight size={16} /></a></> : reviewTask ? <><h2>Semua tugas aktif sudah selesai.</h2><p className="muted">Tinjau kembali tugas terakhir, catatan, checklist, atau tautannya tanpa mengubah XP.</p><a className="btn btn-dark" href={focusTargetHref}><CirclePlay size={18} fill="currentColor" />Tinjau “{reviewTask.text}”<ArrowRight size={16} /></a></> : <><h2>Belum ada misi yang menunggu.</h2><p className="muted">Tambahkan tugas, lalu biarkan TaskFlow membantumu memilih langkah berikutnya.</p><button className="btn btn-dark" type="button" onClick={() => setDialogTask({})}>Buat misi pertama <ArrowRight size={16} /></button></>}</div><Illustration type="focus-run" alt="Ilustrasi sesi fokus" className="mission-illustration" />
      </motion.article>
      <motion.aside className="progress-card" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.42, delay: 0.06 }}><div className="card-topline"><span className="section-kicker">Perjalanan level</span><span className="level-label"><Zap size={14} />Lv {data.progress.level}</span></div><div className="level-number">{data.progress.totalXp}<small>/ {getNextLevelXp(data.progress.totalXp)} XP</small></div><ProgressMeter value={levelProgress} label="Menuju level berikutnya" /><div className="progress-foot"><span><Trophy size={15} />{data.progress.milestones.length} milestone</span><span><Flame size={15} />{data.progress.bestStreak} best streak</span><span title="Sisa streak freeze bulan ini"><Snowflake size={15} />{getStreakFreezesRemaining(data.progress, todayString())} freeze tersisa</span></div></motion.aside>
    </section>
