...
import { applyTaskSave, getCountdownLabel, getCourseProgress, getDashboardStats, getDueInfo, getNextLevelXp, getProfileRecommendations, getStreakFreezesRemaining, getTaskFocusMinutes, getTaskXp, getTodayAgenda, getUpcomingDeadlines, makeTask, selectDailyMission, selectReviewTask, sortTasks, todayString } from '../lib/domain.js';
  ...
  return <>
    ...
    <section className="home-grid">
       <motion.article className={`mission-card ${mission ? '' : 'mission-card-empty'} ${isReviewTarget ? 'mission-card-review' : ''}`} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.36 }}>
        <div className="mission-copy"><div className="card-topline"><span className="eyebrow">{isReviewTarget ? 'Review tugas' : 'Focus Run'}</span><span className="quiet-status"><span className="status-dot" />{isReviewTarget ? 'Tugas sudah selesai' : 'Siap dimulai'}</span></div>{mission ? <><h2>{mission.text}</h2><div className="mission-meta"><span className={`priority-badge priority-${mission.priority}`}>Prioritas {PRIORITY_LABELS[mission.priority].toLowerCase()}</span><span>{getDueInfo(mission).label}</span>{mission.category && <span>{mission.category}</span>}</div><a className="btn btn-dark" href={`focus.html?intent=start&taskId=${mission.id}`}><CirclePlay size={18} fill="currentColor" />Mulai Focus Run<ArrowRight size={16} /></a></> : reviewTask ? <><h2>Semua tugas aktif sudah selesai.</h2><p className="muted">Tinjau kembali tugas terakhir, catatan, checklist, atau tautannya tanpa mengubah XP.</p><a className="btn btn-dark" href={focusTargetHref}><CirclePlay size={18} fill="currentColor" />Tinjau “{reviewTask.text}”<ArrowRight size={16} /></a></> : <><h2>Belum ada misi yang menunggu.</h2><p className="muted">Tambahkan tugas, lalu biarkan TaskFlow membantumu memilih langkah berikutnya.</p><button className="btn btn-dark" type="button" onClick={() => setDialogTask({})}>Buat misi pertama <ArrowRight size={16} /></button></>}</div><Illustration type="focus-run" alt="Ilustrasi sesi fokus" className="mission-illustration" />
      </motion.article>
